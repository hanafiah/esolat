...
class Esolatlocation {

    private $_esolatLocationUrl = 'http://www.e-solat.gov.my/e-solat.php?PageNo={!PAGENO}';
    private $_timeout;
     ...
     * @since 1.0
     */
    public function __construct($timeout = 120) {
    ...
    public function fetchEsolatDom() {
        ...
        $output = array();
        try {
            //get table location paging
            $url = str_replace('{!PAGENO}',1, $this->_esolatLocationUrl);
            curl_setopt($ch, CURLOPT_URL, $url);
            $pagingHtml = curl_exec($ch);
            if ($pagingHtml === false) {
